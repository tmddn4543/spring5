    ...
    $.extend($.jqx._jqxGrid.prototype, {
        ...
        _initmenu: function () {
            ...
            if (this.host.jqxMenu) {
                ...
                var filteritem = $('<li class="filter ' + touchClass + '" style="height: 194px;" ignoretheme="true">' + '<div class="filter"></div>' + '</li>');
        ...
        _rendercolumnheaders: function () {
            ...
            $.each(this.columns.records, function (i, value) {
                ...
                var toggleColumnSelection = function (columnitem, event, select) {
                    if (columnitem.selectable) {
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            self._applycellselection(rows[i].boundindex, columnitem.displayfield, columnitem.selected, false);
                ...
                if (columnitem.checkboxcolumn) {
                    ...
                    checkboxelement.jqxCheckBox({ _canFocus: false, disabled: self.disabled, disabledContainer: true, theme: self.theme, enableContainerClick: false, animationShowDelay: 0, animationHideDelay: 0 });
                ...
                if (filteritem != null) {
                    ...
                    if (self.filterable && self.showfiltermenuitems) {
                        if (!columnitem.filterable) {
                            ...
                            self.gridmenu.height(height + 194);
                            $(filteritem).css('display', 'block');
                        }
                    }
                }
            ...
            if (row != null && cellindex >= 0) {
                ...
                if (column.columntype === "adaptive") {
                    ...
                    for (var i = 0; i < this.columns.records.length; i++) {
                        ...
                        if (this.editable) {
                            if (column.editable) {
                                table += "<td>" + filterIcon + column.text + ": <input data-column='" + column.displayfield + "' value='" + cellValue + "'/></td>";
                            }
                            else {
                                table += "<td>" + filterIcon + column.text + ": <input data-column='" + column.displayfield + "' readonly value='" + cellValue + "'/></td>";
                            }
                        }
                        else {
                            table += "<td>" + filterIcon + column.text + ": " + cellValue + "</td>";
                        }
    ...
    $.jqx.dataview = function () {
        ...
        this.refresh = function (fullupdate) {
            ...
            if (this.rowschangecallback != null) {
                var rows = this.rows;
                if (totalrowsBefore != this.totalrows) this.rowschangecallback({ type: "PagingChanged", data: this.getpagingdetails() });
